...
//float GetInputPixelAlpha(float2 TexCoord)
//{
	//float2 shadowCastersMapRelativeCoord = float2(((renderTargetSizeDesired * TexCoord.x) + lightRelativeZero.x) * shadowCasterMapPixelSize.x, ((renderTargetSizeDesired * TexCoord.y) + lightRelativeZero.y) * shadowCasterMapPixelSize.y);
	//return tex2D(inputSampler, shadowCastersMapRelativeCoord).a;

	//float2 shadowCastersMapRelativeCoord = float2(((renderTargetSizeDesired * TexCoord.x) + lightRelativeZero.x) * shadowCasterMapPixelSize.x, ((renderTargetSizeDesired * TexCoord.y) + lightRelativeZero.y) * shadowCasterMapPixelSize.y);
	//return tex2D(inputSampler, shadowCastersMapRelativeCoord).a;
	//return tex2D(inputSampler, TexCoord).a;
//}
...
float4 DistortAndComputeDistancesPS(float2 TexCoord  : TEXCOORD0) : COLOR0
{
	...
	return float4(distanceV - precision8bitV,precision8bitV * 0.1f,distanceH - precision8bitH,precision8bitH * 0.1f);
}
